...
A λ-system (Dynkin system) that is closed under finite intersections is a σ-algebra (standard fact; see [@folland:real_analysis:1999, §1.2]). We will show that $\lambda(\mathcal{P})$ is closed under finite intersections, leveraging that $\mathcal{P}$ is a π-system.
...
We have shown that $\lambda(\mathcal{P})$ is a λ-system closed under finite intersections; hence it is a σ-algebra.
...
Thus $U \in \sigma(\mathcal{R}_n)$, so $\mathcal{B}(\mathbb{R}^n) \subseteq \sigma(\mathcal{R}_n)$. Note $\mathcal{R}^{\mathbb{Q}}$ is countable (endpoints in $\mathbb{Q}^{2n}$), so $U$ is a countable union of such rectangles.
...
**Problem:** Verify the Carathéodory measurability criterion numerically for a specific set in $\mathbb{R}^2$ (cf. [DEF-1.5.1]).
