...
- **Quick Review of the Week:**
  ...
  - **Day 4:** Carathéodory Extension ([THM-1.4.2], constructing Lebesgue measure rigorously)

- **Key Takeaway:** The σ-algebra $\mathcal{F}$ on a state space $\mathcal{S}$ defines precisely which events are observable. A policy $\pi: \mathcal{S} \to \mathcal{A}$ must be $\mathcal{F}$-measurable to be physically realizable. The Carathéodory construction ([THM-1.4.2]) ensures that transition kernels $P(\cdot | s,a)$ are well-defined probability measures on $(\mathcal{S}, \mathcal{F})$.
...
**Motivation:** We have spent four days constructing the abstract edifice of measure theory: σ-algebras, measures, measurable functions, convergence theorems, and the Carathéodory extension ([THM-1.4.2]). To the pragmatic reader engaged with reinforcement learning algorithms—deep Q-networks, policy gradients, actor-critic methods—this may seem like a detour into mathematical formalism. It is not.
...
**Definition Recap (1.1):** A σ-algebra $\mathcal{F}$ on a set $X$ is a nonempty collection of subsets such that:
1. $X \in \mathcal{F}$ (equivalently, $\emptyset \in \mathcal{F}$)
2. Complementation: $A \in \mathcal{F} \Rightarrow A^c \in \mathcal{F}$
3. Countable unions: $A_1, A_2, \ldots \in \mathcal{F} \Rightarrow \bigcup_{n=1}^{\infty} A_n \in \mathcal{F}$
...
**Example:** For $\mathcal{S} = \mathbb{R}^n$ (continuous control), we use the Borel σ-algebra $\mathcal{B}(\mathbb{R}^n)$, which is generated by open sets. An agent can measure whether the state $s$ belongs to an open region (e.g., "position $> 0$") and, via countable operations, observe all Borel sets. Non‑Borel sets (e.g., Vitali constructions) are not elements of $\mathcal{B}(\mathbb{R}^n)$; membership in such sets is not measurable with respect to the Borel σ‑algebra.
...
- Pointwise limits: if $f_n$ are measurable, then $\lim_{n \to \infty} f_n$ is measurable
...
#### **Pillar 4: Carathéodory Extension Constructs Measures Rigorously** (see [THM-1.4.2])
...
- Carathéodory's theorem ([THM-1.4.2]) guarantees $P(\cdot | s,a)$ is a probability measure on $(\mathcal{S}, \mathcal{B}(\mathcal{S}))$
...
**Definition (Carathéodory Measurability Criterion).** {#DEF-1.5.1}
Let $\mu^*$ be an outer measure on a set $X$. A set $E \subseteq X$ is called **$\mu^*$-measurable** (measurable with respect to $\mu^*$) if for every $A \subseteq X$,
$$
...
|------------------------|-------------------|------------------|---------------------------|
| Carathéodory Extension for general metric spaces | Initial treatment focused on $\mathbb{R}^n$ for concreteness | Day 4, [THM-1.4.2] (proven for $\mathbb{R}^n$) | **Week 10**: General state-space Markov chains (Meyn-Tweedie) |
