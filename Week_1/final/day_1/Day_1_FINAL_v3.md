...
- Numerical reality vs. measurability: hardware computes over floats and finite arrays; non-measurable sets (e.g., Vitali) cannot arise in code, but silent preprocessing (clipping, NaN guards, dataset filters) changes the effective $\sigma$-algebra. Keep transformations continuous/measurable so expectations remain well-defined; see [DEF-1.1.10].
- Policy kernels in practice: we assume standard Borel state/action spaces so transition kernels and stochastic policies are measurable. In code, use library distributions with well-defined densities/support, and prefer continuous squashing (e.g., `tanh`) to maintain measurability through the network.
- Product measures and gradients: many objectives implicitly integrate over $(s,a)$; code rarely annotates $\sigma$-finiteness, but RL math needs it for Fubini–Tonelli and Radon–Nikodym. Ensure state visitation measures and action spaces are $\sigma$-finite in theoretical statements even if the training loop does not expose it explicitly.
- Production note: instrument for numerical pathologies (NaN/Inf hooks, gradient norm clipping). These stabilizers are measurable maps; they trade smoothness for robustness without breaking the measure-theoretic contract required by [THM-1.1.1] and [DEF-1.1.10].
...
**Definition 1.2 (The Borel $\sigma$-Algebra).** {#DEF-1.1.2} For our purposes, the most important measurable space is $(\mathbb{R}, \mathcal{B}(\mathbb{R}))$. The **Borel $\sigma$-algebra**, $\mathcal{B}(\mathbb{R})$, is defined as the *smallest* $\sigma$-algebra on $\mathbb{R}$ that contains all open sets [@folland:real_analysis:1999, §1.1].

**Remark on the Borel $\sigma$-algebra construction.** The rigorous construction of $\mathcal{B}(\mathbb{R})$ from the generating class of open intervals $(a,b)$ uses the **π-λ theorem** (Dynkin's theorem); see **Appendix A2.1** (Week 2, Day 2 supplementary). Readers seeking immediate depth may also consult [@folland:real_analysis:1999, §1.2]. For now, accept that:
- $\mathcal{B}(\mathbb{R})$ exists and is uniquely determined as $\sigma(\{(a,b) : a < b\})$ (see Appendix A2.1)
...
**Definition 1.3 (Measure).** {#DEF-1.1.11} Let $(X, \mathcal{F})$ be a measurable space. A function $\mu: \mathcal{F} \to [0, \infty]$ is a **measure** if it satisfies two conditions:
1.  $\mu(\emptyset) = 0$.
...
**Definition 1.4 (Probability Space).** {#DEF-1.1.3} A measure space $(X, \mathcal{F}, \mu)$ is a **probability space** if $\mu(X) = 1$. The measure $\mu$ is then denoted by $P$ and called a probability measure.

**Example 1.1 (Lebesgue Measure).** {#EX-1.1.1} The canonical measure on $(\mathbb{R}, \mathcal{B}(\mathbb{R}))$ is the Lebesgue measure $\lambda$, which generalizes the notion of length. For any interval $(a,b)$, $\lambda((a,b)) = b-a$.

**Remark on Lebesgue measure construction.** The rigorous construction of Lebesgue measure is achieved via the **Carathéodory extension theorem**, which we prove in **Week 1, Day 4** (Carathéodory Extension). The construction proceeds in five steps:
...
**Definition 1.5 ($\sigma$-Finite Measure).** {#DEF-1.1.4} A measure $\mu$ on $(X, \mathcal{F})$ is **$\sigma$-finite** if there exists a countable collection of sets $\{E_n\}_{n=1}^{\infty} \subseteq \mathcal{F}$ such that $X = \bigcup_{n=1}^{\infty} E_n$ and $\mu(E_n) < \infty$ for all $n \in \mathbb{N}$.

The Lebesgue measure is $\sigma$-finite, as $\mathbb{R} = \bigcup_{n \in \mathbb{Z}} [n, n+1]$.

**Definition 1.6 (Semifinite Measure).** {#DEF-1.1.5} A measure $\mu$ is **semifinite** if for every set $A \in \mathcal{F}$ with $\mu(A) = \infty$, there exists a subset $B \in \mathcal{F}$ such that $B \subseteq A$ and $0 < \mu(B) < \infty$.
...
The class of measurable functions is remarkably stable under common operations. The following results are developed in detail in the companion exercise file `Day_1_exercises_FINAL_v3.md`, with guided proofs.
...
*Sketch of Proof.* For the sum, express $\{x \mid f(x) + g(x) < a\}$ as a countable union over rationals: $\bigcup_{q \in \mathbb{Q}} (\{f < q\} \cap \{g < a-q\})$. For the product, first note $f^2$ is measurable since $\{f^2 < a\}$ is $\emptyset$ for $a \le 0$ and $\{-\sqrt{a} < f < \sqrt{a}\}$ for $a>0$, a preimage of an open set. Then apply the identity $f \cdot g = \frac{1}{2}((f+g)^2 - f^2 - g^2)$. The full proof is given in Exercise 3 of the companion file.
...
*   **Stochastic policies**: In modern RL (policy gradients, entropy-regularized methods), we use stochastic policies $\pi(a|s)$ that specify a probability distribution over actions for each state.
    ...
    Formally, this requires $\pi$ to be a transition kernel from $\mathcal{S}$ to $\mathcal{A}$. A **transition kernel** (formalized Week 5-10) is a family of probability measures $\pi(\cdot|s)$ on $\mathcal{A}$ indexed by $s \in \mathcal{S}$, satisfying measurability conditions: for each Borel set $B \subseteq \mathcal{A}$, the map $s \mapsto \pi(B|s)$ is measurable, ensuring integration operations like $\int_{\mathcal{A}} f(s,a) \pi(da|s)$ are well-defined. We introduce transition kernels rigorously in **Week 5** (for finite Markov chains) and **Week 10** (general state spaces). Stochastic policies in the MDP framework appear in **Week 23** (MDP formalism). The specific role of stochastic policies in the **policy gradient theorem** is developed in Week 24 (Phase V) and Week 35 (Phase VII: Policy Gradient Methods).
...
- **Monte Carlo sampling**: Generate trajectories under policy $\pi$, average the returns (Week 32: Monte Carlo methods)
- **Bootstrapping**: Use Bellman consistency to recursively estimate $V^\pi$ (Week 32-34: TD learning, Q-learning)
- **Function approximation**: Represent $V^\pi$ with neural networks trained on sampled data (Weeks 33-35, 38)
...
3.  The trajectory space $\sigma$-algebra is constructed as the **infinite product $\sigma$-algebra**
   ...
   representing trajectories $(s_0, a_0, s_1, a_1, \ldots)$. We **will** develop this construction rigorously in **Week 4** (Theorem: Kolmogorov Extension) using the Kolmogorov extension theorem, with product $\sigma$-algebras introduced in **Week 2**.
...
*   Formal definition of transition kernels and their measurability properties (Week 5 for finite chains, Week 10 for general state spaces)
*   Construction of product $\sigma$-algebras for trajectory spaces (Week 2)
*   Proof that the Bellman operator $T^\pi V(s) = R(s, \pi(s)) + \gamma \int_{\mathcal{S}} V(s') P(ds'|s, \pi(s))$ is well-defined and contractive on appropriate function spaces (Week 24)
...
Detailed exercises with guided proofs are provided in the companion file: **[[Day_1_exercises_FINAL_v3]]**
