...
Let $s: X \times Y \to [0, \infty)$ be a **simple function**. By definition (Week 1, Day 2), we can write:
$$
s = \sum_{i=1}^{n} a_i \mathbf{1}_{E_i}
$$
where $a_i \geq 0$ and $E_i \in \mathcal{F}_X \otimes \mathcal{F}_Y$ are disjoint measurable sets.

**Key observation:** Every set $E \in \mathcal{F}_X \otimes \mathcal{F}_Y$ is a countable union of disjoint rectangles (this follows from the semiring structure and σ-algebra generation). For simplicity, we first establish Tonelli for simple functions of the form $s = \sum_{i=1}^{n} a_i \mathbf{1}_{A_i \times B_i}$ where $A_i \times B_i$ are disjoint rectangles. (The general case follows by approximating arbitrary measurable sets by rectangles, a technical detail we defer to Folland §2.4, Lemma 2.36.)
...
**Applying MCT to sections:** Fix $x \in X$. For each $n$, the section $s_n(x, \cdot): Y \to [0, \infty)$ is a simple function (by Step 2), hence measurable. By Step 2, we have:
$$
...
**Proposition 2.6 (Tonelli for Sums and Series).** Let $(X, \mathcal{F}_X, \mu)$ and $(Y, \mathcal{F}_Y, \nu)$ be σ-finite measure spaces. Let $\{f_{ij}\}_{i,j \in \mathbb{N}}$ be a double sequence of non-negative measurable functions $f_{ij}: X \times Y \to [0, \infty]$. Then:
...
print(f"\nOuter integral: ∫_X F1(x) dμ(x) = {result_1:.6f}")
...
print(f"\nOuter integral: ∫_Y F2(y) dν(y) = {result_2:.6f}")
...
print(f"Double integral: {result_3:.6f}")

# Verification
print("\n" + "="*60)
print("VERIFICATION:")
print("="*60)
print(f"Iterated (Y first, X outer): {result_1:.6f}")
print(f"Iterated (X first, Y outer): {result_2:.6f}")
print(f"Double integral:              {result_3:.6f}")
print(f"\nAll equal? {np.allclose([result_1, result_2, result_3], result_1)}")
...
print(f"\nSum of contributions: {np.sum(integrand):.6f} = {result_3:.6f} ✓")
```
